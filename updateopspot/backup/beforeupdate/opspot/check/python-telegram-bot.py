...
downloadscriptpath='/home/pi/scripts/opspot/check/python-telegram-bot.py'
...
class Logger(object):    # calling print will show in logs
    def __init__(self):
        self.terminal = sys.stdout
        self.log = open("/home/pi/scripts/opspot/logs/python-log.txt", "a")
...
sys.stdout = Logger()
print ("started Logger: viewable at bot.log")
    ...
    try:
        ...
        if outputp == "stream":
            ...
            if switched2stream == False:
                #make bot.send_message function
                bot.send_chat_action(chat_id=chatid, action=telegram.ChatAction.TYPING)
            ...
            os.rename('media/requested_live.jpg', 'media/prev_requested_live.jpg')
        elif outputp == "local":
        ...
        bot.send_message(chat_id=chatid, text="You don't have a last requested photo! Sorry")
...
def checkforupdate(bot, update):
    global user, chatid, scriptpath, downloadscriptpath 
    print("UPDATE: User requested update")
    canupdate = "none"
    try:
        bot.send_chat_action(chat_id=chatid, action=telegram.ChatAction.TYPING)
        bot.send_message(chat_id=chatid, text="Checking for update, one moment...")
        updatecommand = ["rclone", "copy", "-v", "gdrive:packages/update/opspot/python-telegram-bot.py", "/home/pi/scripts/testopspotupdate"]
    ...
    if canupdate is "new":
        ...
        print("canupdate!=new")
        print("UPDATE: Error downloading latest telegram-python-bot.py from remote gdrive")
